...
public class ModelAttributeMethodProcessor implements HandlerMethodArgumentResolver, HandlerMethodReturnValueHandler {
	...
	public final Object resolveArgument(MethodParameter parameter, @Nullable ModelAndViewContainer mavContainer,
		...
		// 如果model中已经有这个名字的对象了, 那么就使用这个
		// 比如前面通过在某个方法上贴上@ModelAttribute, 并且有返回值, 则会往model中添加对象, 那么这里能拿到值, 就从model里面取出来使用
		if (mavContainer.containsAttribute(name)) {
			attribute = mavContainer.getModel().get(name);
		}

		// 要是model中没有这个值, 那么就新创建
		else {
		...
		if (bindingResult == null) {
			...
			// 为每一个参数new一个ExtendedServletRequestDataBinder对象
			// 执行贴了注解@InitBinder的方法, 执行条件为:
			// 贴了注解@InitBinder未指定value名字的, 那么此方法会执行
			// 贴了注解@InitBinder("xxx")指定了名字, 并且和当前HandlerMethod参数上@ModelAttribute("xxx")一致的才会执行此方法
			WebDataBinder binder = binderFactory.createBinder(webRequest, attribute, name);
			if (binder.getTarget() != null) {
				if (!mavContainer.isBindingDisabled(name)) {

					// 获取url中的参数, 原生键值存取到PropertyValue中, 如u.username=1
					// 调用贴了注解@InitBinder的方法, 并获取用户注入的前缀
					// 取出老的PropertyValue, 去除前缀u.后, PropertyValue和对象的属性名是一致的了, 然后使用新的PropertyValue
					// 内省的方式直接注入属性
