...
public class WebDataBinder extends DataBinder {
	...
	protected void doBind(MutablePropertyValues mpvs) {
		...
		// 把mpvs中, 每一个PropertyValue中name带前缀的, 全部转为不带前缀的, 如u.username转为username
	...
	protected void checkFieldDefaults(MutablePropertyValues mpvs) {
		...
		// 获取binder.setFieldDefaultPrefix("u.");的前缀u.
		String fieldDefaultPrefix = getFieldDefaultPrefix();
		if (fieldDefaultPrefix != null) {

			// 这个是从request中获取的原生参数, 如: u.username=2, 键为u.username, 值为2
			PropertyValue[] pvArray = mpvs.getPropertyValues();
			for (PropertyValue pv : pvArray) {

				// 获取u.开头的前缀, 把原来的u.username键去除前缀, 变成username
				if (pv.getName().startsWith(fieldDefaultPrefix)) {

					// 这个字段是截取u.username字符串后的username
