...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		// 在web中默认的Spring容器为XmlWebApplicationContext
		...
		// 这个配置文件就是contextConfigLocation, 是在DispatcherServlet的静态代码块中加载的
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		// 这步很重要, 添加SourceFilteringListener监听器, 当容器初始化完毕后, 对SpringMVC的默认类进行加载并初始化为bean
		...
		if (env instanceof ConfigurableWebEnvironment) {
			// initPropertySources会在refresh中调用, 在这里先调用是因为, 在refresh可能会先依赖这个方法
